...
assert.deepEqual(groups.normalize(JSON.parse(JSON.stringify(state)),blueprint),{groups:state.groups,seatGroups:state.seatGroups,heldSeats:state.heldSeats,groupLinks:state.groupLinks,seatColors:{}});
assert.deepEqual(groups.normalize({assignments:{}},blueprint),{groups:[],seatGroups:{},heldSeats:{},groupLinks:[],seatColors:{}});
const paintSource=groups.clone(state);paintSource.assignments['L1-A-01']={status:'assigned',name:'검증',fixed:true};
const painted=groups.paint(paintSource,['L1-A-01','C-A-05'],'#12abcd',blueprint);
assert.deepEqual(painted.assignments,paintSource.assignments);assert.deepEqual(painted.seatGroups,paintSource.seatGroups);assert.deepEqual(painted.heldSeats,paintSource.heldSeats);
assert.equal(paintSource.seatColors,undefined);assert.equal(painted.seatColors['L1-A-01'],'#12abcd');
assert.deepEqual(groups.normalize(JSON.parse(JSON.stringify(painted)),blueprint).seatColors,painted.seatColors);
assert.equal(groups.paint(painted,['L1-A-01'],null,blueprint).seatColors['L1-A-01'],undefined);
assert.equal(groups.paint(painted,['L1-A-01'],null,blueprint).seatColors['C-A-05'],'#12abcd');
assert.throws(()=>groups.paint(state,['INVALID'],'#123456',blueprint));
assert.throws(()=>groups.normalize({...state,seatColors:{'C-A-01':'red'}},blueprint));
assert.throws(()=>groups.normalize({...state,seatColors:{'INVALID':'#123456'}},blueprint));
